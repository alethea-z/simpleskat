...
<head>
  ...
  <style>
    :root { color-scheme: light; --bg:#f7faf7; --panel:#fff; --text:#17301c; --muted:#55705a; --border:#d7e6da; --pass-bg:#e7f8ea; --pass-text:#12632a; --pass-border:#9ed7ab; --fail-bg:#fef2f2; --fail-text:#991b1b; --fail-border:#fca5a5; }
      ...
      <section class="scenario">
        ...
        <div class="steps">
          ${scenario.steps.map((step) => `
            <article class="step">
              <strong>${escapeHtml(step.stepText)}</strong>
              <div class="muted">${escapeHtml(step.status)} · ${escapeHtml(step.screenshotFile)}</div>
              <img src="./${escapeHtml(step.screenshotFile)}" alt="${escapeHtml(step.stepText)}">
            </article>
          `).join('')}
        </div>
      </section>`).join('')}
